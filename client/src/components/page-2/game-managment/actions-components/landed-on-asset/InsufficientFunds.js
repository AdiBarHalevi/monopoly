...
const InsufficientFunds = (props) => {
  ...
  if (!sellAssetState)
    return (
      <div>
        you do no have sufficient funds for this purchase would you like to sell
        assets or go to an auction
        <button>go to auction</button>{" "}
        <button onClick={changeSellAssetState}>sell assets</button>
      </div>
    );
