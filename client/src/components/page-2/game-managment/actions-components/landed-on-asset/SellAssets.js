...
import {mortgageAnAssetAPI,updatedGameBoardData,retirePlayer} from "../../../../../axioscall"
...
const SellAsset = (props)=>{
    ...
    <Container>
        {activeUserDataState.property.map((asset,i)=>{
            ...
            return <CardShow key ={i+20}>
                <CardHeader color={gameboardDataState[asset.fieldNum][`headerColor`]}></CardHeader>
                <h4>{gameboardDataState[asset.fieldNum][`name`]}</h4>
                <div>Assets to mortgage: {gameboardDataState[asset.fieldNum][`property`][0][`Assets`]}</div>
                <div>mortgage value: ${gameboardDataState[asset.fieldNum][`price`]*0.6}</div>
                <Button onClick={()=>mortgageaAssets(asset)}>mortgage the asset</Button>
            </CardShow>
        })}
        </Container>
